...
const DISCOGS_KEY = env.DISCOGS_CONSUMER_KEY;
const DISCOGS_SECRET = env.DISCOGS_CONSUMER_SECRET;
...
export async function fetchRelease(resourceUrl: string): Promise<RecordInfo | null> {
...
export async function fetchReleasePrice(releaseId: number): Promise<number | null> {
  const url = `https://api.discogs.com/releases/${releaseId}?key=${DISCOGS_KEY}&secret=${DISCOGS_SECRET}`;
  const res = await fetch(url, { headers });
  if (!res.ok) {
    console.error(`[discogs] price fetch failed | ${res.status} ${res.statusText} | releaseId=${releaseId}`);
    return null;
  }
  const release: DiscogsRelease = await res.json();
  return release.lowest_price ?? null;
}

export async function searchByBarcode(barcode: string): Promise<RecordInfo | null> {
